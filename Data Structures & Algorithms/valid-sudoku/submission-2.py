class Solution:
    def isValidSudoku(self, board: List[List[str]]) -> bool:
        ...
        #####################
        # mistakes : a few logic errors when it came to searching columns and rows:
            # forgot to account for "."
            # when searching columns, used the wrong counter to refer to the columns and rows 
        
            # optimisations : my initial solution was to manually generate an array that stored all the squares, and check for duplicates in rows, columns and square separately
            
            # fix : the more efficient and readable thing to do is have 1 for loop for checking rows, and within that check columns and squares as well
            # columns need a nested for loop to check
            # squares use a dictionary that stores square id and the numbers in the square (using a set as it doesn't allow duplicates)
            # for each number, check which square it is (check logic in code) and if the number is already there, then the grid is invalid

        # if all the code runs without returning false, return true as it works
        ######################
