class Solution:
    def characterReplacement(self, s: str, k: int) -> int:
        ...
        ### why don't we decrease maxFreq when removing a majority char
        # we use maxFreq monotonically instead of actual max frequency, we only update it when there is a new char that has a greater frequency of chars
        # this means that when we shrink, we don't always end up with a valid window
        # but that doesn't matter because we have already found a window that is the same size or larger and is definitely valid when that maxFreq was genuinely present
        # we update length continuously
